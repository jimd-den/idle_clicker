...
This initial version (Version 1.0) of Non-Idle Clicker will focus on the core functionality of work unit tracking, time measurement, and basic note-taking.

**In Scope:**

*   Work unit (click) tracking.
*   Elapsed time measurement (stopwatch).
*   Clicks Per Minute (CPM) calculation and display.
...
*   **FR-PLAY-1: CPM Display:** The application shall display the calculated Clicks Per Minute (CPM) in real-time or near real-time, updating as the user clicks.
*   **FR-PLAY-2: Stopwatch Display:** The application shall display an elapsed time stopwatch, showing minutes and seconds (MM:SS) format.
*   **FR-PLAY-3: Timer Start/Reset on Screen Load:** The stopwatch timer shall start automatically when the Play Screen is loaded. The timer should reset to 00:00 when the screen is revisited after being paused or navigating away and back.
...
*   **FR-PLAY-9: Note-Taking on Pause:** When the timer is paused, the application shall allow the user to input timestamped notes related to the task.
    *   **FR-PLAY-9.1: Note Input Field:** Provide a text input field for note entry.
    *   **FR-PLAY-9.2: Timestamping:**  Automatically timestamp each note with the current elapsed time when the note is saved or added.
    *   **FR-PLAY-9.3: Note Storage:** Store notes locally, associated with the current session.
    *   **FR-PLAY-9.4: Note Display (Basic):**  Display notes in a simple list format below the input field, showing the timestamp and note text.

### 5.2. Data Management

*   **FR-DATA-1: Local Storage:** All application data, including session time, click counts, and notes, shall be stored locally on the user's device.
*   **FR-DATA-2: Session Persistence (Temporary):**  Session data (time, clicks, notes) should persist while the application is in the foreground or background. Closing the application completely may reset the session (to be clarified - for V1.0, resetting on app close is acceptable for simplicity).
*   **FR-DATA-3: Data Reset (Manual):** Provide a clear way for the user to manually reset the current session data (timer, clicks, notes) on the Play Screen.
...
*   **NFR-USAB-2: Clear Information Display:**  Information displayed (CPM, time, clicks, notes) should be clear, concise, and easily readable.
...
*   **NFR-SEC-1: Local Data Storage Security:** Data is stored locally and relies on the device's inherent security. No specific application-level security measures are required for Version 1.0 beyond standard mobile app security practices.
...
*   **Application Layer (Use Cases/Interactors):**  JavaScript/TypeScript modules (potentially in a `use_cases/` or `services/` directory - to be determined) that contain the application's business logic and orchestrate interactions between the UI and the Domain Layer.  For Version 1.0, this might be relatively thin, primarily managing the WorkTimerContext.
*   **Domain Layer (Entities and Business Rules):** JavaScript/TypeScript modules (potentially in a `domain/` or `entities/` directory - to be determined) that define the core business entities (e.g., `WorkSession`, `Note`) and business rules. For Version 1.0, this might be simplified and primarily managed within the `WorkTimerContext`.
*   **Infrastructure Layer (Frameworks and Drivers):**  React Native framework, device APIs, and local storage mechanisms.  Context API (`contexts/WorkTimerContext.tsx`) acts as a form of infrastructure for state management in Version 1.0.
...
*   **Language:** TypeScript
*   **State Management:** React Context API (`contexts/WorkTimerContext.tsx`) (for Version 1.0 - consider more robust solutions like Zustand or Redux for future versions if complexity increases).
*   **Styling:** React Native StyleSheet, Themed Components (`components/Themed*`)
*   **Local Storage:** React Native AsyncStorage (or similar - for future persistence beyond session).
*   **UI Components:** React Native core components, custom components (`components/`)

## 9. Future Considerations (Beyond Version 1.0)

*   **Data Persistence:** Implement persistent storage of session history, allowing users to review past work sessions and track progress over time.
...
This Software Requirements Specification serves as a guide for the development of Non-Idle Clicker Version 1.0. It will be reviewed and updated as needed throughout the development process.
